...
class ArrayValidator
{
    ...
    protected function validateRootFilter(): void
    {
        ...
        if ($rootFilter === null || count($rootFilter) === 0) return;
    ...
    protected function validateFilter(array $filter, $address): void
    {
        ...
        foreach (['f', 'field', 'and', 'group_and', 'or', 'group_or'] as $key){
            if(array_key_exists($key, $filter)) $foundKeys[] = $key;
        }

        if(count($foundKeys) === 0) throw new JoryException('A filter should contain one of the these fields: "f", "field", "and", "group_and", "or" or "group_or". (Location: ' . $address . ')');
        if(count($foundKeys) > 1) throw new JoryException('A filter cannot contain more than one of the these fields: "f", "field", "and", "group_and", "or" or "group_or". (Location: ' . $address . ')');
        ...
        if ($foundKey === 'f' || $foundKey === 'field') {
            ...
            if (array_key_exists('o', $filter) && array_key_exists('operator', $filter)) throw new JoryException('A filter cannot contain both an "o" and "operator" parameter, remove one. (Location: ' . $address . ')');
            if (array_key_exists('v', $filter) && array_key_exists('value', $filter)) throw new JoryException('A filter cannot contain both an "v" and "value" parameter, remove one. (Location: ' . $address . ')');

            // A string value in "field" (or "f") is required
            if (!is_string($filter[$foundKey])) throw new JoryException('The "' . $foundKey . '" parameter should have a string value. (Location: ' . $address . ')');

            // A string value in "operator" (or "o") is optional
            if ($this->hasArrayKey($filter, ['o', 'operator']) && !is_string($this->getArrayValue($filter, ['o', 'operator']))) throw new JoryException('The "operator" (or "o") parameter should have a string value or be omitted. (Location: ' . $address . ')');
        ...
        if ($foundKey === 'and' || $foundKey === 'group_and') {
            ...
            if(!is_array($subFilters)) throw new JoryException('The "' . $foundKey . '" parameter should hold an array with filters. (Location: ' . $address . ')');
