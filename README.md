...
# Jory
Jory is a highly flexible way of defining database queries into a JSON string, useful for loading dynamic data from the front-end. Jory can add high flexibility to your REST API without the need to create i.e. a whole Graphql implementation. It can easily be used next to your existing code.

This package only translates a Jory JSON string into PHP objects, and thus defines the convention for setting up Jorystrings. This convention is explained further in this document, for the Laravel implementation for setting up a Jory endpoint take a look at the [laravel-jory](https://packagist.org/packages/joskolenberg/laravel-jory) package.

## The basics
A Jory-string is a JSON string consisting of 5 parts, all these parts are optional.
...
* Order these records by number_of_no_1_hits ascending first, en by id ascending second.
...
### Parameters
All parameters have a full and minfied version. For example; "flt" is the shorthand notation for the "filter" parameter. All shorthand notations will be shown in parentheses after the full name in this document: ```filter (flt)```
...
The ```operator (o)``` and ```data (d)``` are optional (depening on the implementation). Only passing a ```field (f)``` could be useful in boolean conditions, when only passing a ```field (f)``` and ```data (d)``` parameter a fallback to an "equals" comparison should be implemented by the API.
...
This can be done using the ```group_and (and)``` or ```group_or (or)``` parameters. The parameters should contain an array of filters, these filters can be [single filters](#single-filters) or another grouped AND or OR filter if you want to create nested conditions. 
...
Return only the records which are active AND are last modified in 2017:
```json
{
  "filter": {
    "group_and": [
...
Order the result by last_name first and first_name ascending second (minified):
```json
{
  "srt": [
    "last_name",
...
Any suggestions or issues? Please contact me!
